...
**Membora** es una plataforma web SaaS responsive para gimnasios, centros fitness y estudios deportivos pequenos o medianos. Es una aplicacion de gestion para propietarios, recepcion, comerciales, entrenadores y administradores de la plataforma.
...
Aplicación: https://membora.es/app/
...
- Canal de novedades con version actual de la plataforma e historial de cambios.
- Panel de administracion de Membora separado en resumen, contactos, empresas, usuarios, cobros, facturas, planes y logs; el diagnostico de web/correo esta oculto del menu.
...
- Web comercial en `/` y aplicación en `/app/`, bajo el mismo dominio.
...
|   |-- demo.html
|-- apps/
|   |-- crm/                     # aplicación privada (PHP; ruta técnica conservada)
...
| `https://membora.es/app/` | Aplicación | Mismo `httpdocs`, mediante `httpdocs/app/index.php` |
...
7. Abrir `https://membora.es/` para la web y `https://membora.es/app/` para la plataforma.

No hay que ejecutar comandos Node, compilar frontend ni reiniciar una app Node.

No hay que editar tokens en la web. El formulario envia al webhook de la plataforma y las solicitudes aparecen en `Administración Membora > Contactos`.
...
Los enlaces de demo de la web publica no abren una maqueta estatica: crean un usuario temporal unico e inician una sesion real de la plataforma durante 20 minutos sin depender del nombre exacto de `APP_ENV`. El acceso publico solo habilita la demo cliente; la demo de administrador queda limitada al entorno interno `demo`. El usuario se elimina al cerrar sesion, al caducar o tras la senal de cierre de pestana; la limpieza por caducidad actua tambien como respaldo. Al finalizar, la plataforma devuelve al usuario a `WEB_APP_URL`.

La web tambien ofrece una prueba gratuita self-service de 14 dias. El alta requiere verificar el email; despues crea o actualiza el contacto como `Cliente` con el nombre de la empresa y el correo introducido, lo vincula a una empresa `TRIAL` y crea el tenant y su usuario administrador. Se genera una contrasena aleatoria y se envia un segundo correo con un enlace temporal: la credencial esta cifrada, solo se revela tras una confirmacion y no puede volver a verse despues. Las solicitudes mantienen validacion de origen y honeypot. El limite por IP y email esta temporalmente desactivado por defecto y puede recuperarse con `TRIAL_RATE_LIMIT_ENABLED=true`. Se recomienda configurar `APP_KEY`; los despliegues antiguos usan de forma compatible la credencial estable de base de datos para derivar la clave.

En el `.env` de la plataforma debe existir `APP_URL="https://membora.es/app"` y
...
- Alta manual de contactos comerciales antes de crear su plataforma.
- Tabla `empresas`.
...
- `Mejorar el plan` marca el plan actual y solo permite ascensos. En el proveedor simulado, Basic puede subir a Pro, Business o Enterprise; Pro a Business o Enterprise; y Business a Enterprise. Stripe Checkout permite la misma progresión desde cuentas sin una suscripción Stripe vinculada y evita crear suscripciones duplicadas. El webhook y el retorno verificado sincronizan el plan de forma idempotente. Los controles técnicos y diagnósticos Stripe siguen ocultos en las pantallas administrativas.
